...
const synergyCommercialActions = [
...
const SectionHeader = ({ icon: Icon, title, subtitle, right }) => (
  <div className="p-6 border-b border-slate-100 flex items-start justify-between gap-3">
    <div className="flex items-center gap-3">
      ...
      <div>
        <div className="text-lg font-black tracking-tight text-slate-900 leading-tight">{title}</div>
...
const Field = ({ label, hint, children }) => (
  <div className="space-y-2">
    <div className="flex items-end justify-between gap-2">
      ...
      {hint ? <div className="text-[11px] text-slate-400 italic">{hint}</div> : null}
    </div>
    {children}
  </div>
);
...
const Input = ({ value, onChange, placeholder, className = "" }) => (
  <input
    ...
    className={`w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:bg-white focus:ring-2 focus:ring-emerald-200 transition-all ${className}`}
  />
);
...
const TextArea = ({ value, onChange, placeholder, rows = 4 }) => (
  <textarea
    ...
    placeholder={placeholder}
    rows={rows}
    className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:bg-white focus:ring-2 focus:ring-emerald-200 transition-all resize-none"
  />
);
...
const Select = ({ value, onChange, options }) => (
  <div className="relative">
    <select
      value={value}
      onChange={(e) => onChange(e.target.value)}
      className="w-full appearance-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none focus:bg-white focus:ring-2 focus:ring-emerald-200 transition-all"
    >
      {options.map((op) => (
...
const Chip = ({ active, onClick, children }) => (
  <button
    ...
    onClick={onClick}
    className={[
      "inline-flex items-center gap-2 rounded-full border px-3 py-2 text-xs font-black transition-all",
      active
...
const CardRow = ({ title, subtitle, children, onRemove, removable = true }) => (
  <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm hover:border-emerald-200 transition-colors">
    <div className="flex items-start justify-between gap-3">
      ...
      {removable ? (
        <button
          type="button"
          onClick={onRemove}
          className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 hover:bg-red-50 hover:text-red-600 hover:border-red-100 transition-colors"
          title="Eliminar"
        >
...
const StepPortfolio = ({ data, updateCard, addCard, removeCard, showToast }) => {
...
const StepMatriz = ({ data, updateCard, addCard, removeCard, showToast }) => {
...
const StepIntervenciones = ({ data, updateCard, addCard, removeCard, showToast }) => {
...
const StepSinergias = ({ data, update, updateCard, addCard, removeCard, showToast }) => {
...
const StepCharlas = ({ data, updateCard }) => (
  <Card>
    <SectionHeader
      icon={Users}
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <Field label="Rol">
              <Select
                ...
                options={["A evaluar", "Disertante", "Sponsor", "Validación", "Ensayo"]}
              />
            </Field>
...
const StepServicios = ({ data, update }) => (
  <Card>
    <SectionHeader
      icon={Leaf}
...
const StepFinal = ({ data, update, sendToWhatsApp, resetAll }) => (
  <Card>
    <SectionHeader
      icon={Send}
...
export default function App() {
  ...
  const initialData = {
    ...
    objetivoOtro: "",
    kpis: "",
    portfolio: [{ producto: "", tipo: "", cultivosObjetivo: "", diferencial: "", estado: "Consolidado" }],
    matrizTecnica: [{ cultivo: "", problema: "", mesCritico: "Septiembre", solucion: "", formato: "Charla" }],
    intervenciones: [{ mes: "Septiembre", cultivo: "", tema: "MIP", temaOtro: "", producto: "", formato: "Charla técnica", origen: "Estratégico" }],
    sinergiasTecnicasTipos: [],
    sinergiaTecnicaOtro: "",
    combosTecnicos: [{ componentes: "", beneficio: "", cultivo: "", formato: "Demo" }],
    sinergiasComercialesAcciones: [],
    sinergiaComercialOtro: "",
    sinergiasComercialesNotas: "",
    participacionCharlas: talkTracks.map((t) => ({ tema: t, interes: "A evaluar", rol: "A evaluar", comentarios: "" })),
    aporteProductos: "",
    aporteEvidencias: "",
    indicadoresImpacto: [],
    indicadorOtro: "",
  ...
  const addCard = (field, card) =>
    setData((d) => ({ ...d, [field]: [...d[field], card] }));
  ...
  const buildReport = () => {
    ...
    let lines = [];
    lines.push("*COMUNIDAD +VOS — ESTRATEGIA 2026*");
    lines.push("");
    lines.push(`*Empresa:* ${data.empresa || "—"}`);
    lines.push(`*Resp:* ${data.responsable || "—"} (${data.cargo || "—"})`);
    lines.push(`*Contacto:* ${data.email || ""} ${data.telefono || ""}`);
    lines.push(`*Zona:* ${data.zona}`);
    lines.push("");
    lines.push("*1. OBJETIVOS Y KPI*");
    ...
    data.portfolio.forEach((p, i) => {
      ...
      lines.push(`${i + 1}. ${p.producto} (${p.tipo}) - ${p.estado}`);
    ...
    data.matrizTecnica.forEach((r, i) => {
      ...
      lines.push(`${i + 1}. ${r.cultivo}: ${r.problema} en ${r.mesCritico}`);
    ...
    data.intervenciones.forEach((r, i) => {
      if (!isFilled(r.cultivo)) return;
      const tema = r.tema === "Otro" ? r.temaOtro : r.tema;
      lines.push(`${i + 1}. ${r.mes} - ${r.cultivo}: ${tema}`);
    ...
    data.combosTecnicos.forEach((c, i) => {
      if (!isFilled(c.componentes)) return;
      lines.push(`   Combo ${i + 1}: ${c.componentes} -> ${c.beneficio}`);
    });
    ...
    data.participacionCharlas.forEach((p) => {
      if (p.interes === "Sí" || p.rol !== "A evaluar") {
        lines.push(`- ${p.tema}: ${p.interes} / ${p.rol}`);
        if (p.comentarios) lines.push(`  (${p.comentarios})`);
      }
    });
    lines.push("");
  ...
  const sendToWhatsApp = () => {
    if (!isFilled(data.empresa)) {
      showToastFn("Por favor complete el nombre de la Empresa.");
      setStep(1); // Ir a perfil
      return;
    }
  ...
  const stepsList = [
    ...
    { key: "sinergias", label: "Sinergias", icon: HeartHandshake },
